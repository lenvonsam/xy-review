    ...
    swiper-slide(v-show="custNo.length != 0")
      .banner-bg.border-box.full-width.pt-2rem.padding-xl.slide-third-bg.relative(:style="{height: screenHeight + 'px'}")
        ...
        .row.mt-15.text-center
          div.margin-auto.text-white.ml-23l
            //- label
            //-   input(type="radio", name="inputRadio", v-model="getGood")
            //-   span.ml-8 是
            //- label.ml-23
            //-   input(type="radio", name="inputRadio", v-model="getGood")
            //-   span.ml-8 否
            label.ml-23(v-for="goods,index in getGoodsGroup", :key="index",style="display: flex;flex-direction: row;position:relative")
              input(type="radio", v-model="getGoods", :value="goods.id")
              span.ml-8 {{goods.name}}
        .row.mt-20
