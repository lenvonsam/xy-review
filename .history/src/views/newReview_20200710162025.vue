...
.container.ls-1(v-show="loadFinish") 
  ...
  swiper.swipper.full-width(:options="swiperOption", ref="xySwiper", @slideChange="slideTouchChange") 
    swiper-slide(v-show="custNo.length != 0")
    ...
    swiper-slide(v-show="custNo.length != 0")
      .banner-bg.box.full-width.relative.slide-fifth-bg.relative(:style="{height: screenHeight + 'px'}")
        .slide-fifth-book.banner-bg
          div.content-style.ff-zwj
            .mt-15 没有了电话里的讨价还价
            .mt-10 没有了到处比价的捉摸不定
            .row.mt-10.align-center
              div 有的只是您下单时
              .circle-bkg
                .text-red.title.lh-33(class="animated bounceInLeft") 运筹帷幄
              .col 的气势
            .mt-10 不忘采购的初心，更不负挣钱的韶华
            .mt-10 型云心目中2020年第二季度的 
            .row.mt-10.align-center
              .circle-bkg             
                .text-red.title.lh-33(class="animated bounceInLeft") 华东钢皇
              .col 非你莫属!
      ...
      .banner-bg.border-box.full-width.pt-2rem.padding-xl.slide-third-bg.relative(:style="{height: screenHeight + 'px'}")
        ...
        .row.mt-15.text-center
          div.margin-auto.text-white
            label
              input(type="radio", name="inputRadio", ref="getGood", value="1")
              span.ml-8 是
            label.ml-23
              input(type="radio", name="inputRadio", ref="getGood", value="0")
              span.ml-8 否
        .row.mt-20
          button.submit-btn(type='submit', @click="submitGood") 确定
        .mt-20.relation-bkg
          .goods-first {{reviewObj.goods_name_one}}
          .goods-second {{reviewObj.goods_name_two}}
          .goods-third {{reviewObj.goods_name_three}}
          .goods-forth {{reviewObj.goods_name_four}}
          .goods-fifth {{reviewObj.goods_name_five}}
      .black.arrow.banner-bg.animated.infinite.zoomIn(style="animation-duration: 2s")
    swiper-slide(v-show="custNo.length != 0")
...
class NewReviewPage extends Vue {
  ...
  async submitGood() {
    debugger;
    console.log(this.$refs.getGood.value);
  }
  async submintInfor() {
